...
Since we’ll be using the width and height of our screen a lot throughout this series, we will add them as 2 variables to the **Properties** section of our code:
...
The last line asks the SpriteBatch to draw the backgroundTexture image, spanning the entire window. We’ll discuss the last argument in the next chapter, for now understand that **Color.White** means that the image should be rendered in its original colors. The code line adds the background image to the list of images the SpriteBatch has to draw (a list of only 1 image).
