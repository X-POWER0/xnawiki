...
Let’s start by downloading these files, you can find them by clicking on this link and on this link. To save them on your computer, right-click on one of them, select “Save image as” and save it to a location on your computer. Make sure you know the location where you save it.

Now go to MonoGame Game Studio 4.0, and find your Solution Explorer. It’s the small rectangle at the top-right of your screen, containing all files of your solution and project. Next, find the Content project in the Solution Explorer, right-click on it and select Add -> Existing Item, as shown in the image below.
...
With the image loaded into our MonoGame project, we should create a variable in our code, to which we will link the image. The variable is required, so we can access the image from within our code.
...
Or we specify a rectangle on the screen where we want the image to be rendered into. In case the size of your image doesn’t match the size of the rectangle, MonoGame will take care that the image is scaled so it fits in perfectly.
